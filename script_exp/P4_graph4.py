...
def show(simMatrix):
    ...
    with open("../output/graph_4/graph_4_SimRank.txt", "w") as output:
        for i, n1 in enumerate(nodes):
            for n2 in nodes[i:]:
...
def Visual(Dict, ntop=5, xlabel='Node', ylabel='Value', title='Authority'):
    ...
    with open("../output/graph_4/graph_4_{:s}.txt".format(title), "w") as output:
        for i in range(ntop):
            output.write( '{:0.5f}'.format(Dict[top[i]]) + ' ')
    a=np.loadtxt("../output/graph_4/graph_4_{:s}.txt".format(title))
    print(title +':')
    print (a)    
